$( "#form" ).submit(function( event ) {
		...
		if (f == "") {
			...
			msg_error = msg_error + '\nInserire il nome di una farmacia';
		...
		if (nif == "") {
			...
			msg_error = msg_error + '\nInserire un NIF della farmacia';
		...
		if (!x) {
			...
			msg_error = msg_error + '\nÈ necessario accettare le condizioni di utilizzo';
        ...
        error: function (jqXHR, status) {
			...
			if (codigo == 409) {
				const div = document.getElementById('modalText');
				const header = document.getElementById('modalHeader');
				div.textContent = 'La registrazione non può essere effettuata più di 3 volte';
				header.textContent = 'ERROR';
				event.preventDefault();
				$('#myModal').modal('show');
				...
				const div = document.getElementById('modalText');
				const header = document.getElementById('modalHeader');
				div.textContent = 'Si è verificato un errore nella registrazione';
				header.textContent = 'ERROR';
				event.preventDefault();
				$('#myModal').modal('show');
				...
				const div = document.getElementById('modalText');
				const header = document.getElementById('modalHeader');
				div.textContent = 'La registrazione è stata inviata con successo';
